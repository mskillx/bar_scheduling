...
export default function Schedule() {
  ...
  const events = shifts.map((s) => {
    ...
    return {
      ...
      backgroundColor: isMine ? color : color + "28",
      borderColor: isMine ? color : color + "70",
      extendedProps: { shift: s, isMine, color },
    };
  });
      ...
      <div className="card flex-1 overflow-hidden p-0">
        <div className="p-4 h-full">
          <FullCalendar
            ...
            eventContent={(info) => {
              const isMine: boolean = info.event.extendedProps.isMine;
              const color: string = info.event.extendedProps.color;
              return (
                <div className="p-1 overflow-hidden text-xs">
                  <div className="font-medium truncate" style={{ color: isMine ? "white" : color }}>
                    {info.event.title}
                  </div>
                  <div
                    style={{
                      color: isMine ? "rgba(255,255,255,0.7)" : color + "aa",
                    }}
                  >
                    {format(info.event.start!, "HH:mm").replace(":", ".")} –{" "}
                    {info.event.end ? format(info.event.end, "HH:mm").replace(":", ".") : ""}
                  </div>
                </div>
              );
            }}
