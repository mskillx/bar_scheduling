...
function UserFormModal({
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['users'] }); toast.success(t('user.created')); onClose() },
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['users'] }); toast.success(t('user.updated')); onClose() },
    onError: (e: any) => toast.error(e.response?.data?.detail || t('user.failed')),
  })
...
export default function UsersPage() {
  ...
  const disableMutation = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['users'] }); toast.success(t('user.disabledSuccess')) },
  })
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-dark-600">
                ...
                <th className="text-left px-6 py-3 text-gray-400 font-medium">{t('user.status')}</th>
                <th className="text-left px-6 py-3 text-gray-400 font-medium">{t('user.actions')}</th>
              </tr>
            </thead>
                    ...
                    <div className="flex gap-2">
                      ...
                      {u.active && (
                        <button
                          ...
                          onClick={() => {
                            if (confirm(t('user.disableConfirm', { name: u.first_name }))) disableMutation.mutate(u.id)
                          }}
                        >
                          {t('user.disable')}
                        </button>
