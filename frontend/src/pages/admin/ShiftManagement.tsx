...
export default function ShiftManagement() {
  ...
  const copyPreviousWeek = async () => {
    ...
    const prevShifts = await shiftsApi.list({
            ...
            >
              <div className="flex items-center justify-between mb-2">
                ...
                <div className="flex gap-1">
                  ...
                  {clipboard &&
                    format(clipboard.day, "yyyy-MM-dd") !== format(day, "yyyy-MM-dd") && (
                      <button
        ...
        <div className="card p-0 overflow-hidden">
          <div className="px-6 py-4 border-b border-dark-600">
            ...
            <p className="text-xs text-gray-500 mt-0.5">
              {t("shiftManagement.hourlyCoverageDesc")}
            </p>
          </div>
            ...
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b border-dark-600">
                  <th className="text-left px-4 py-2 text-gray-500 w-14">
                    {t("shiftManagement.hour")}
                  </th>
                  {days.map((d) => (
                    <th
                      key={d.toISOString()}
                      className="text-center px-2 py-2 text-gray-400 font-medium"
                    >
                      {format(d, "EEE dd")}
                    </th>
                  ))}
                  ...
                  <tr key={hour} className="border-b border-dark-700/40">
                    ...
                    {days.map((_, di) => {
                      ...
                      return (
                        <td
                          key={di}
                          className={`text-center py-1.5 transition-colors ${coverageCellClass(count)}`}
                        >
      ...
      >
        {editShift && (
          <ShiftForm
            defaultValues={{
              ...editShift,
              notes: editShift.notes ?? undefined,
            }}
            onSubmit={(data) =>
              updateMutation.mutate({
                id: editShift.id,
                data: data as ShiftUpdate,
              })
            }
